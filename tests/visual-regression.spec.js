...
test.describe('Violet Void Theme - Visual Regression', () => {
  ...
  test('ArchWiki - Homepage', async ({ page }) => {
    ...
    // Take screenshot
    await page.screenshot({ path: 'screenshots/wiki-homepage.png', fullPage: true });
  });
  ...
  test('ArchWiki - Installation Guide', async ({ page }) => {
    ...
    await page.screenshot({ path: 'screenshots/wiki-installation-guide.png', fullPage: true });
  });
  ...
  test('ArchWiki - Table Rendering', async ({ page }) => {
    ...
    if (tableCount > 0) {
      ...
      const hasRowspan = await firstTable.locator('th[rowspan], td[rowspan]').count();
      const hasColspan = await firstTable.locator('th[colspan], td[colspan]').count();

      console.log(`Table has rowspan: ${hasRowspan > 0}, colspan: ${hasColspan > 0}`);
    }

    await page.screenshot({ path: 'screenshots/wiki-tables.png', fullPage: true });
  });
  ...
  test('ArchWiki - Search Functionality', async ({ page }) => {
    ...
    if (await searchInput.count() > 0) {
      ...
      // Check search button
      const searchButton = await page.locator('.cdx-search-input__end-button').first();
      if (await searchButton.count() > 0) {
        const bgColor = await searchButton.evaluate(el => getComputedStyle(el).backgroundColor);
        console.log('Search button background:', bgColor);
      }
    }

    await page.screenshot({ path: 'screenshots/wiki-search.png', fullPage: true });
  });
  ...
  test('ArchWiki - Code Block with Copy Button', async ({ page }) => {
    ...
    if (await codeBlock.count() > 0) {
      ...
      const copyButton = await page.locator('.copy-to-clipboard, .copy-button').first();
  ...
  test('AUR - Homepage', async ({ page }) => {
    ...
    // Check search input
    const searchInput = await page.locator('#pkgsearch input[type="text"]').first();
    if (await searchInput.count() > 0) {
      const bgColor = await searchInput.evaluate(el => getComputedStyle(el).backgroundColor);
      console.log('AUR search input background:', bgColor);
    }

    await page.screenshot({ path: 'screenshots/aur-homepage.png', fullPage: true });
  });
  ...
  test('AUR - Package Search Results', async ({ page }) => {
    ...
    await page.screenshot({ path: 'screenshots/aur-search-results.png', fullPage: true });
  });
  ...
  test('AUR - Package Details', async ({ page }) => {
    ...
    await page.screenshot({ path: 'screenshots/aur-package-details.png', fullPage: true });
  });
  ...
  test('Forums - Homepage', async ({ page }) => {
    ...
    if (await forumList.count() > 0) {
      const bgColor = await forumList.evaluate(el => getComputedStyle(el).backgroundColor);
      console.log('Forum item background:', bgColor);
    }

    await page.screenshot({ path: 'screenshots/forums-homepage.png', fullPage: true });
  });
  ...
  test('Forums - Topic View', async ({ page }) => {
    ...
    if (postCount > 0) {
      ...
      const postBg = await firstPost.evaluate(el => getComputedStyle(el).backgroundColor);
      console.log('Post background:', postBg);

      // Check code blocks in posts
      const codeBlock = await firstPost.locator('.code, pre').first();
      if (await codeBlock.count() > 0) {
        const codeBg = await codeBlock.evaluate(el => getComputedStyle(el).backgroundColor);
        console.log('Code in post background:', codeBg);
      }
    }

    await page.screenshot({ path: 'screenshots/forums-topic.png', fullPage: true });
  });
  ...
  test('Interaction - Link Hover States', async ({ page }) => {
    ...
    if (await link.count() > 0) {
      const beforeHover = await link.evaluate(el => getComputedStyle(el).color);
  ...
  test('Quirk Detection - Font Size Issues', async ({ page }) => {
    ...
    const smallFonts = await page.evaluate(() => {
      const allElements = Array.from(document.querySelectorAll('p, span, div, a, li'));
      const small = [];
