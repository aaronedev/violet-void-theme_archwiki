...
function brightnessFromRgb(backgroundColor) {
  ...
  return rgb.slice(0, 3).map(Number).reduce((sum, value) => sum + value, 0);
}
...
async function expectActionableInstallState(page) {
  ...
  await expect
    .poll(
      () => installButton.evaluate((button) => typeof button.onclick === 'function'),
      {
        timeout: 15000,
        message: 'Expected Stylus to attach the install handler before clicking',
      }
    )
    .toBe(true);
}
...
test.describe('Stylus Extension Integration', () => {
  ...
  test('Install theme into Stylus and verify ArchWiki', async ({ context: { context } }) => {
    ...
    console.log('[TEST] Opening Stylus via the supported in-extension installer path...');
    ...
    await expect
      .poll(async () => {
        ...
        persistedStyle = await currentBackground.evaluate(async (updateUrl) => {
          const styles = await API.styles.getAll();
          const match = styles.find((style) => style.updateUrl === updateUrl);
        ...
        timeout: 15000,
        message: 'Expected Stylus background state to contain the installed style record',
      })
      .toBe(1);
    ...
    await page.waitForURL(new RegExp(`chrome-extension://${extensionId}/manage\\.html`), {
      timeout: 15000,
    });
