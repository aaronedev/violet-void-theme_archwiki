...
(async () => {
  ...
  await page.goto('https://wiki.archlinux.org/title/Installation_guide', { waitUntil: 'networkidle' });
  ...
  const checks = await page.evaluate(() => {
    ...
    const dropdowns = Array.from(document.querySelectorAll('[class*="dropdown"], [class*="menu"], [id*="menu"], [id*="dropdown"]'));
    ...
    const searchInputs = Array.from(document.querySelectorAll('input[type="search"], input[type="text"][name*="search"], .cdx-search-input'));
    ...
    results.horizontalOverflow = document.body.scrollWidth > document.body.clientWidth;
    ...
    // Check search button
    const searchBtns = Array.from(document.querySelectorAll('button, [role="button"], .cdx-button'));
