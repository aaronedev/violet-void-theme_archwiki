...
(async () => {
  ...
  const states = [
    ...
    { name: 'menu-open', fn: async () => {
      ...
      const menuCheckbox = page.locator('#vector-main-menu-dropdown-checkbox').first();
    ...
    { name: 'toc-open', fn: async () => {
      ...
      const tocToggle = page.locator('#toc-toggle-button, .toc-toggle button, button[data-target="toc"], .vector-toc-toggle-button').first();
      if (await tocToggle.count() > 0) {
        await tocToggle.click();
        await page.waitForTimeout(300);
      }
    }},
    { name: 'search-active', fn: async () => {
      // Click the search input to activate it
      const searchInput = page.locator('.cdx-search-input__input, #searchInput, input[name="search"]').first();
  ...
  async function resetStates() {
    try {
      await page.locator('#vector-main-menu-dropdown-checkbox').first().uncheck({ force: true });
    } catch(e) {}
    try {
      await page.locator('#toc-toggle-button, .toc-toggle button').first().click();
    } catch(e) {}
    try {
  ...
  for (const p of pages.slice(0, 2)) {
    for (const s of mobileStates) {
      try {
        await mobilePage.goto(p.url, { waitUntil: 'networkidle', timeout: 30000 });
        await mobilePage.addStyleTag({ path: './dist/main.css' });
  ...
  results.filter(r => r.status === 'ok').forEach(r => {
    try {
      const hash = execSync(`md5sum .agent/archwiki/current/${r.path.split('/').pop()}`).toString().slice(0, 32);
  ...
  if (uniqueHashes < totalOk) {
    ...
    Object.entries(hashes).forEach(([hash, files]) => {
      if (files.length > 1) console.log(`  Duplicate hash ${hash}: ${files.join(', ')}`);
    });
  } else {
