...
(async () => {
  ...
  const states = [
    ...
    { name: 'toc-open', fn: async (p) => {
      // Click TOC toggle if present
      const toggle = p.locator('.vector-toc-toggle, #toc-toggle, [data-target="toc"]').first();
      if (await toggle.count() > 0) {
        await toggle.click();
        await p.waitForTimeout(400);
      }
    }},
    { name: 'search-active', fn: async (p) => {
      // Focus the search input
      const search = p.locator('#searchInput, .vector-search-box-input, [name="search"]').first();
  ...
  for (const pg of pages) {
    for (const s of states) {
      try {
        await desktopPage.goto(pg.url, { waitUntil: 'domcontentloaded', timeout: 15000 });
        ...
        if (s.name === 'menu-open') {
          try {
            const cb = desktopPage.locator('#vector-main-menu-dropdown-checkbox');
        ...
        results.push({ page: pg.name, state: s.name, status: 'error', error: e.message.slice(0, 120) });
      }
    }
  }
  ...
  const mobileStates = [
    ...
    { name: 'menu-open', fn: async (p) => {
      // Mobile hamburger
      const btn = p.locator('.vector-sticky-pinning-container, .vector-main-menu-trigger, #mw-editableNavigation').first();
  ...
  for (const pg of pages.slice(0, 2)) {
    for (const s of mobileStates) {
      try {
        await mobilePage.goto(pg.url, { waitUntil: 'domcontentloaded', timeout: 15000 });
        ...
        console.log(`OK: ${safeName}`);
        results.push({ page: pg.name, state: s.name, status: 'ok', path, mobile: true });
        ...
        console.log(`ERR: ${pg.name}.mobile.${s.name} - ${e.message.slice(0, 80)}`);
        results.push({ page: pg.name, state: s.name, status: 'error', error: e.message.slice(0, 120) });
      }
    }
  }
  ...
  console.log('\n=== SUMMARY ===');
  console.log(`Total: ${results.length}, OK: ${results.filter(r => r.status === 'ok').length}, Errors: ${results.filter(r => r.status === 'error').length}`);
  
  // Write results
  const fs = require('fs');
  fs.writeFileSync('.agent/archwiki/reports/scout-results.json', JSON.stringify(results, null, 2));
