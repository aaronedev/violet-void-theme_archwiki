...
async function checkNavIssues(page) {
  return await page.evaluate(() => {
    ...
    const navItems = Array.from(
      document.querySelectorAll('.mw-sidebar a, #mw-sidebar a, nav a, .nav a')
    ).slice(0, 20)
    ...
    const menus = document.querySelectorAll(
      '.dropdown, .menu, .popup, [role="menu"], [role="navigation"]'
    )
...
async function checkTOC(page) {
  return await page.evaluate(() => {
    ...
    const toc = document.querySelector(
      '.toc, #toc, [role="navigation"][aria-label="Table of contents"]'
    )
    if (toc) {
    ...
    const collapsibles = document.querySelectorAll(
      '.mw-collapsible, [aria-expanded], details'
    )
...
async function checkSearchState(page) {
  return await page.evaluate(() => {
    ...
    const searchInputs = document.querySelectorAll(
      'input[type="search"], input[name="search"], #searchInput, .cdx-search-input__input'
    )
...
async function checkHamburgerMenu(page) {
  return await page.evaluate(() => {
    ...
    const toggles = document.querySelectorAll(
      '.mw-ui-button, #mw-sidebar-button, [aria-label="Menu"], .menu-toggle, .hamburger, #n-shownav'
    )
...
async function checkCodeBlocks(page) {
  return await page.evaluate(() => {
    ...
    const codeBlocks = Array.from(
      document.querySelectorAll('pre, code, .mw-highlight')
    )
...
async function run() {
    ...
    try {
      ...
      try {
        ...
        const menuToggle = page
          .locator(
            '.mw-ui-button, #mw-sidebar-button, [aria-label="Menu"], .menu-toggle, #n-shownav'
          )
          .first()
      ...
      try {
        ...
        const tocLink = page
          .locator('a[href="#toc"], a[href*="toctitle"]')
          .first()
      ...
      try {
        ...
        console.log(
          `  ! Search not interactable: ${e.message.substring(0, 50)}`
        )
      }
      ...
      try {
        const mobileToggle = page
          .locator('.mw-ui-button, [aria-label="Menu"], .menu-toggle')
          .first()
  ...
  const realFindings = findings.filter(
    (f) => !f.error && f.issues && f.issues.length > 0
  )
  ...
  if (realFindings.length === 0) {
    ...
    for (const finding of realFindings) {
      ...
      for (const issue of finding.issues) {
        console.log(
          `  - [${issue.type}] ${JSON.stringify(issue).substring(0, 100)}`
        )
      }
    }
  }
